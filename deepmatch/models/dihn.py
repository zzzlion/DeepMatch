"""
Reference:
https://zhuanlan.zhihu.com/p/474637882
https://link.zhihu.com/?target=https%3A//github.com/EzailShen/WWW-22-DIHN
...
def model_fn(features, label_dict, fc_generator, is_training, keep_prob, params):
    ...
    with tf.variable_scope("user_intent_net"):
        ...
        #  behavior squence： 部分trigger特征以及行为序列做attention得到加权结果
...
class SelfAttentionPooling:
    ...
    可以参考 https://blog.csdn.net/qq_35169059/article/details/101678207
    """
    ...
    def multi_head(self, q, k, v, scope):
        with tf.variable_scope(scope):
            q, k, v = self._linear_projection(q, k, v)  # 默认Q,K,V 维度相同 [B,S,56]
    ...
    def _split_heads(self, q, k, v):
        def split_last_dimension_then_transpose(tensor, num_heads, dim):
            t_shape = tensor.get_shape().as_list()  # [B,max_seq_len,56]
            tensor = tf.reshape(tensor, [-1] + t_shape[1:-1] + [num_heads, dim // num_heads])  # [-1,S,head,56//head]
    ...
    def _scaled_dot_product(self, qs, ks, vs):
        ...
        o1 = tf.matmul(qs, ks, transpose_b=True)  # [B,HEAD,max_seq_len,E] * [B,HEAD,max_seq_len,E]^T
        ...
        o2 = o1 / (key_dim_per_head ** 0.5)  # (batch_size, num_heads, q_length, k_length)  这里q,k length指的是这个句子真实的长度，而不是padding到n
        # key_mask 0-1矩阵, 标识K中的真实值
        # 对Q*K结果中K-padding部分赋一个很负的数，则过softmax后对应attention权重会趋近于0; Q-padding位置直接置为0即可
        if self.key_mask is not None:  # (batch_size, k_length)
            ...
            mask = tf.tile(mask, [1, self.num_heads, 1, 1])  # (batch_size, num_heads, q_length, k_length)
